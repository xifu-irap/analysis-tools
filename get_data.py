...
# -----------------------------------------------------------------------------
def read_data_ascii(dumpfilename, config):
    r"""
        This function reads data from a DRE dump file in ascci format,
        and returns 1 array (format <h).
        The file header and the dumptype (2 firsts 32-bit words) are removed from the data
    ...
    fdat=open(fullfilename, 'r')
    data=np.array(()).astype('<h')
    for line in fdat:
        data=np.append(data, converter.signed_hexa_2_dec(line[:4],16))
        data=np.append(data, converter.signed_hexa_2_dec(line[4:],16))
    fdat.close()
    print(data[0:10])
    #data=np.fromfile(fdat, dtype='<h')
