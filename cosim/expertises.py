...
def sequence_delay(path, filename_err, error_delay_dict, col=0):
    ...
    Args:
        ...
        error_delay_dict (dictionary) with:
...
def dac_delay(path, filename_err, filename_dac, dac_delay_dict, col=0):
    ...
    Args:
        ...
        dac_delay_dict (dictionary): with signal_name (string) and parameter_name (string)
        col (int, optional): index of the column to be characterized. Defaults to 0.
    """
    ...
    print("    The edge of the", dac_delay_dict["signal_name"],
          "signal after delay correction is on sample {0:d}\n".format(x_edge_dac_corrected))
...
def amp_sq_vphi(path, filename, setup, col=0):
    ...
    Args:
        ...
        setup (dictionary): contains the setting of the expertise measurement
        col (int, optional): index of the column to be characterized. Defaults to 0.
    """
    ...
    v_offset_low = (1 / 6 * (setup["start_value"] + np.arange(n_steps) * setup["step_size"])
                    * cst.amp_sq_quantum * 2 ** correction_factor)
