...
# -----------------------------------------------------------------------
def make_plots(pixel, data_tes, data_error, data_feedback, data_science, x_range, ticks_ratio, plotfilename):
    ...
    major_ticks = np.arange(0, xmax, 4*ticks_ratio)
    minor_ticks = np.arange(0, xmax, 1*ticks_ratio)
        ...
        for pixel in range(mux_factor):
            ...
            if data_tes[pixel].max()>0 and abs(data_error[pixel]).max()>0 \
                and data_feedback[pixel].max()>0 and data_science[pixel].max()>0:
                    ...
                    # Plot close to the pulse
                    x_range=[xmin, xmax]
                    ticks_ratio=50
                    plotfilename=os.path.join(plotdirname, "pix-{0:1d}".format(pixel).zfill(2)+"_pulse.png")
                    make_plots(pixel, data_tes, data_error, data_feedback, data_science, x_range, ticks_ratio, plotfilename)

                    # Plot close to the start
                    x_range=[x_zoom_min, x_zoom_max]
                    ticks_ratio=1
                    plotfilename=os.path.join(plotdirname, "pix-{0:1d}".format(pixel).zfill(2)+"_start.png")
                    make_plots(pixel, data_tes, data_error, data_feedback, data_science, x_range, ticks_ratio, plotfilename)
